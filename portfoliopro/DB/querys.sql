...
WHERE p1.user_id = u.id AND p1.portfolio_name = "port1" AND p2.portfolio_id = p1.id AND i.project_id = p2.id

--유저의 포트폴리오 목록
SELECT p1.portfolio_name, p1.portfolio_image_path 
FROM users u, portfolios p1
WHERE p1.user_id = u.id AND u.id = 1
LIMIT 12
...
--프로필 이미지, 컨텐츠 변경
UPDATE users
SET user_image_path = "1", user_profile_content = "1"
WHERE id=1

--포트폴리오 이미지, 컨텐츠 변경
UPDATE portfolios
SET portfolio_image_path = "1", portfolio_content = "1"
WHERE id=1

--프로젝트 이미지, 컨텐츠 변경, 추가
UPDATE projects
SET project_content = "1"
WHERE id=1
